...
def extract_caller_callee_info(entry, pass_name):
    ...
    if "Args" in entry:
        for arg in entry["Args"]:
            ...
            if "Callee" in arg:
                if "DebugLoc" in arg:
                    #print ("Debug location", arg["DebugLoc"]["File"])
                    callee_filename = arg["DebugLoc"]["File"]
                callee = arg["Callee"]
                break
    
    return caller, callee, tag, callee_filename

def group_by_callee_with_callers(entries, pass_name):
    """Group by callee, storing (caller, tag) pairs"""
    grouped = defaultdict(list)
    callee_filename_list = set()
    
    for e in entries:
        res = extract_caller_callee_info(e, pass_name)
        if res is not None:
            caller, callee, tag, callee_filename = res
            if callee:
                grouped[callee].append((caller, tag))
                if callee_filename is not None:
                    callee_filename_list.add(os.path.abspath(os.path.normpath(callee_filename)))
        else:
            caller, callee, tag, callee_filename = None, None, None, None
    
    return grouped, callee_filename_list
...
def process_single_file(args):
    ...
    try:
        ...
        grouped, callee_filename_list = group_by_callee_with_callers(entries, pass_name)
        
        local_func_count = [0, 0]
        # [mixed_count, same_count]
        ...
        #print (callee_filename_list)
        return local_func_count, callee_filename_list
...
def main_parallel(yaml_dir, pass_name, num_processes=None):
    ...
    # Separate the results into func_counts and filename_lists
    func_count_results = []
    func_callee_filename_list = []
    ...
    for result in results:
        if result is None:
            ...
            func_count, filename_set = result
            func_count_results.append(func_count)
            func_callee_filename_list.append(filename_set)
    
    # Check if any file triggered the exit condition
    if None in func_count_results:
        print("Exit condition met - 'Analysis' found in tags")
        sys.exit(0)
    
    # Aggregate func_count_results
    total_func_count = [0, 0]
    total_func_callee_filename_list = set()
    for result in func_count_results:
        total_func_count[0] += result[0]
        total_func_count[1] += result[1]
    for result in func_callee_filename_list:
        total_func_callee_filename_list.update(result)
    
    end_time = time.time()
    print(f"\nProcessing completed in {end_time - start_time:.2f} seconds")
    
    return total_func_count, total_func_callee_filename_list

def main_sequential(yaml_dir, pass_name):
    """Original sequential processing for comparison"""
    all_files = find_opt_files(yaml_dir)
    func_count = [0, 0]
    total_callee_filename_list = set()
    start_time = time.time()
    ...
    for i, filename in enumerate(all_files):
        ...
        try:
            entries = load_yaml_entries(filename)
            grouped, callee_filename_list = group_by_callee_with_callers(entries, pass_name)
            total_callee_filename_list.update(callee_filename_list)
    ...
    return func_count, total_callee_filename_list
...
if __name__ == "__main__":
    ...
    if use_parallel:
        ...
        func_count, callee_filename_list = main_parallel(yaml_dir, pass_name, num_processes)
    else:
        print("Running sequential version...")
        func_count, callee_filename_list = main_sequential(yaml_dir, pass_name)
    ...
    #print (callee_filename_list)
    callee_filename_lists = list(callee_filename_list)
    callee_filename_lists.sort()
    #print (callee_filename_lists)
    fp = open(pass_name + "_callee_names.txt", "w" )
    for filename in callee_filename_lists:
        fp.write(filename + "\n")
    fp.close()