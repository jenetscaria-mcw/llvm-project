...
bool GIMatchTableExecutor::executeMatchTable(
  ...
  __attribute__((always_inline)) auto handleReject = [&]() -> RejectAction {
  ...
  __attribute__((always_inline)) const auto propagateFlags = [&]() {
    for (auto MIB : OutMIs) {
  ...
  __attribute__((always_inline)) const auto getTypeFromIdx = [&](int64_t Idx) -> LLT {
    if (Idx >= 0)
      return ExecInfo.TypeObjects[Idx];
    return State.RecordedTypes[1 - Idx];
  };

  __attribute__((always_inline)) const auto readULEB = [&]() {
  ...
  // negative values.
  __attribute__((always_inline)) const auto readS8 = [&]() { return (int8_t)MatchTable[CurrentIdx++]; };

  __attribute__((always_inline)) const auto readU16 = [&]() {
    auto V = readBytesAs<uint16_t>(MatchTable + CurrentIdx);
    CurrentIdx += 2;
    return V;
  };

  __attribute__((always_inline)) const auto readU32 = [&]() {
    auto V = readBytesAs<uint32_t>(MatchTable + CurrentIdx);
    CurrentIdx += 4;
    return V;
  };

  __attribute__((always_inline)) const auto readU64 = [&]() {
