...
#define LLVM_MI_ASMPRINTERFLAGS_BITS 8
  ...
  public:
    ...
    __attribute__((always_inline)) ArrayRef<MachineMemOperand *> getMMOs() const {
    ...
    __attribute__((always_inline)) MDNode *getPCSections() const {
    ...
    __attribute__((always_inline)) MDNode *getMMRAMetadata() const {
  ...
  private:
    ...
    __attribute__((always_inline)) size_t numTrailingObjects(OverloadToken<MachineMemOperand *>) const {
      return NumMMOs;
    }
    __attribute__((always_inline)) size_t numTrailingObjects(OverloadToken<MCSymbol *>) const {
...
public:
  ...
  __attribute__((always_inline)) const MachineBasicBlock* getParent() const { return Parent; }
  __attribute__((always_inline)) MachineBasicBlock* getParent() { return Parent; }
  ...
  __attribute__((always_inline)) MachineFunction *getMF() {
  ...
  __attribute__((always_inline)) uint32_t getFlags() const {
    return Flags;
  }

  /// Return whether an MI flag is set.
  __attribute__((always_inline)) bool getFlag(MIFlag Flag) const {
  ...
  __attribute__((always_inline)) void setFlags(unsigned flags) {
  ...
  __attribute__((always_inline)) bool isBundled() const {
    return isBundledWithPred() || isBundledWithSucc();
  }

  /// Return true if this instruction is part of a bundle, and it is not the
  /// first instruction in the bundle.
  __attribute__((always_inline)) bool isBundledWithPred() const { return getFlag(BundledPred); }

  /// Return true if this instruction is part of a bundle, and it is not the
  /// last instruction in the bundle.
  __attribute__((always_inline)) bool isBundledWithSucc() const { return getFlag(BundledSucc); }
  ...
  __attribute__((always_inline)) const DebugLoc &getDebugLoc() const { return DbgLoc; }
  ...
  __attribute__((always_inline)) const MCInstrDesc &getDesc() const { return *MCID; }

  /// Returns the opcode of this MachineInstr.
  __attribute__((always_inline)) unsigned getOpcode() const { return Opcode; }

  /// Retuns the total number of operands.
  __attribute__((always_inline)) unsigned getNumOperands() const { return NumOperands; }
  ...
  __attribute__((always_inline)) const MachineOperand& getOperand(unsigned i) const {
    assert(i < getNumOperands() && "getOperand() out of range!");
    return Operands[i];
  }
  __attribute__((always_inline)) MachineOperand& getOperand(unsigned i) {
    assert(i < getNumOperands() && "getOperand() out of range!");
    return Operands[i];
  }
  ...
  __attribute__((always_inline)) ArrayRef<MachineMemOperand *> memoperands() const {
    if (!Info)
      return {};
  ...
  __attribute__((always_inline)) mmo_iterator memoperands_begin() const { return memoperands().begin(); }
  ...
  __attribute__((always_inline)) bool hasOneMemOperand() const { return memoperands().size() == 1; }

  /// Return the number of memory operands.
  __attribute__((always_inline)) unsigned getNumMemOperands() const { return memoperands().size(); }
  ...
  /// instruction bundles.
  __attribute__((always_inline)) bool hasProperty(unsigned MCFlag, QueryType Type = AnyInBundle) const {
    assert(MCFlag < 64 &&
  ...
  /// legalization steps.
  __attribute__((always_inline)) bool isPreISelOpcode(QueryType Type = IgnoreBundle) const {
  ...
  __attribute__((always_inline)) bool mayRaiseFPException() const {
  ...
  __attribute__((always_inline)) bool isCopy() const {
