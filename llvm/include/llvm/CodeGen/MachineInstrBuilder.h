...
public:
  __attribute__((always_inline)) MachineInstrBuilder() = default;

  /// Create a MachineInstrBuilder for manipulating an existing instruction.
  /// F must be the machine function that was used to allocate I.
  __attribute__((always_inline)) MachineInstrBuilder(MachineFunction &F, MachineInstr *I) : MF(&F), MI(I) {}
  MachineInstrBuilder(MachineFunction &F, MachineBasicBlock::iterator I)
      : MF(&F), MI(&*I) {}

  /// Allow automatic conversion to the machine instruction we are working on.
  __attribute__((always_inline)) operator MachineInstr*() const { return MI; }
  __attribute__((always_inline)) MachineInstr *operator->() const { return MI; }
  operator MachineBasicBlock::iterator() const { return MI; }

  /// If conversion operators fail, use this method to get the MachineInstr
  /// explicitly.
  __attribute__((always_inline)) MachineInstr *getInstr() const { return MI; }

  /// Get the register for the operand index.
  /// The operand at the index should be a register (asserted by
  /// MachineOperand).
  __attribute__((always_inline)) Register getReg(unsigned Idx) const { return MI->getOperand(Idx).getReg(); }

  /// Add a new virtual register operand.
  const MachineInstrBuilder &addReg(Register RegNo, unsigned flags = 0,
                                    __attribute__((always_inline)) unsigned SubReg = 0) const {
    assert((flags & 0x1) == 0 &&
  ...
  __attribute__((always_inline)) const MachineInstrBuilder &addImm(int64_t Val) const {
    MI->addOperand(*MF, MachineOperand::CreateImm(Val));
    return *this;
  }

  __attribute__((always_inline)) const MachineInstrBuilder &addCImm(const ConstantInt *Val) const {
    MI->addOperand(*MF, MachineOperand::CreateCImm(Val));
    return *this;
  }

  __attribute__((always_inline)) const MachineInstrBuilder &addFPImm(const ConstantFP *Val) const {
    MI->addOperand(*MF, MachineOperand::CreateFPImm(Val));
    return *this;
  }

  const MachineInstrBuilder &addMBB(MachineBasicBlock *MBB,
                                    __attribute__((always_inline)) unsigned TargetFlags = 0) const {
  ...
  __attribute__((always_inline)) const MachineInstrBuilder &addMemOperand(MachineMemOperand *MMO) const {
  ...
  __attribute__((always_inline)) const MachineInstrBuilder &add(const MachineOperand &MO) const {
    MI->addOperand(*MF, MO);
    return *this;
  }
  ...
  __attribute__((always_inline)) const MachineInstrBuilder &addIntrinsicID(Intrinsic::ID ID) const {
  ...
  __attribute__((always_inline)) const MachineInstrBuilder &setMIFlags(unsigned Flags) const {
    MI->setFlags(Flags);
    return *this;
  }
  ...
  bool constrainAllUses(const TargetInstrInfo &TII,
                        ...
                        __attribute__((always_inline)) const RegisterBankInfo &RBI) const {
