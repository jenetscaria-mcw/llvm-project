...
public:
 ...
 inline __attribute__((always_inline)) static constexpr LLT scalar(unsigned SizeInBits) {    return LLT{/*isPointer=*/false, /*isVector=*/false, /*isScalar=*/true,
  ...
  inline __attribute__((always_inline)) explicit constexpr LLT()
  : IsScalar(false), IsPointer(false), IsVector(false), RawData(0) {}
  explicit LLT(MVT VT);

  inline __attribute__((always_inline)) constexpr bool isValid() const { return IsScalar || RawData != 0; } 
  inline __attribute__((always_inline)) constexpr bool isScalar() const { return IsScalar; }  
  constexpr bool isToken() const { return IsScalar && RawData == 0; };
  constexpr bool isVector() const { return isValid() && IsVector; }
  inline __attribute__((always_inline)) constexpr bool isPointer() const {    return isValid() && IsPointer && !IsVector;
  }
  ...
  /// vector types.
  inline __attribute__((always_inline)) constexpr bool isScalable() const {    
  ...
  inline __attribute__((always_inline)) constexpr ElementCount getElementCount() const {    
  ...
  inline __attribute__((always_inline)) constexpr TypeSize getSizeInBits() const {    
  ...
  inline __attribute__((always_inline)) constexpr TypeSize getSizeInBytes() const {    
  ...
  inline __attribute__((always_inline)) constexpr unsigned getScalarSizeInBits() const {    
    if (IsScalar)
  ...
  inline __attribute__((always_inline)) constexpr unsigned getAddressSpace() const {    
  ...
  inline __attribute__((always_inline)) constexpr bool operator==(const LLT &RHS) const {    
    return IsPointer == RHS.IsPointer && IsVector == RHS.IsVector &&
           IsScalar == RHS.IsScalar && RHS.RawData == RawData;
  }

  inline __attribute__((always_inline)) constexpr bool operator!=(const LLT &RHS) const { return !(*this == RHS); }
...
private:
  ...
  inline __attribute__((always_inline)) static constexpr uint64_t getMask(const BitFieldInfo FieldInfo) {    
    const int FieldSizeInBits = FieldInfo[0];
    return (((uint64_t)1) << FieldSizeInBits) - 1;
  }
  inline __attribute__((always_inline)) static constexpr uint64_t maskAndShift(uint64_t Val, uint64_t Mask,
          uint8_t Shift) {    assert(Val <= Mask && "Value too large for field");
    return (Val & Mask) << Shift;
  }
  inline __attribute__((always_inline)) static constexpr uint64_t maskAndShift(uint64_t Val,
           const BitFieldInfo FieldInfo) {    return maskAndShift(Val, getMask(FieldInfo), FieldInfo[1]);
  }

  inline __attribute__((always_inline)) constexpr uint64_t getFieldValue(const BitFieldInfo FieldInfo) const {    return getMask(FieldInfo) & (RawData >> FieldInfo[1]);
  }

  inline __attribute__((always_inline)) constexpr void init(bool IsPointer, bool IsVector, bool IsScalar,
                      ElementCount EC, uint64_t SizeInBits,
      unsigned AddressSpace) {    
...
public:
 inline __attribute__((always_inline)) constexpr uint64_t getUniqueRAWLLTData() const {    return ((uint64_t)RawData) << 3 | ((uint64_t)IsScalar) << 2 |
...
template<> struct DenseMapInfo<LLT> {
  ...
  inline __attribute__((always_inline)) static bool isEqual(const LLT &LHS, const LLT &RHS) {    return LHS == RHS;
  }
};
