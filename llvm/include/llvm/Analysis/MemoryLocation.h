...
class LocationSize {
 ...
 inline __attribute__((always_inline)) constexpr LocationSize(uint64_t Raw, DirectConstruction) : Value(Raw) {}  constexpr LocationSize(uint64_t Raw, bool Scalable)
...
public:
  ...
  inline __attribute__((always_inline)) constexpr LocationSize(uint64_t Raw)
   : Value(Raw > MaxValue ? AfterPointer : Raw) {}  // Create non-scalable LocationSize
 inline __attribute__((always_inline)) static LocationSize precise(uint64_t Value) {    return LocationSize(Value, false /*Scalable*/);
  }
 inline __attribute__((always_inline)) static LocationSize precise(TypeSize Value) {    return LocationSize(Value.getKnownMinValue(), Value.isScalable());
  }
  ...
  /// underlying object).
 inline __attribute__((always_inline)) constexpr static LocationSize beforeOrAfterPointer() {    return LocationSize(BeforeOrAfterPointer, Direct);
  }
 ...
 inline __attribute__((always_inline)) bool hasValue() const {    return Value != AfterPointer && Value != BeforeOrAfterPointer;
  }
 inline __attribute__((always_inline)) bool isScalable() const { return (Value & ScalableBit); }
 inline __attribute__((always_inline)) TypeSize getValue() const {    assert(hasValue() && "Getting value from an unknown LocationSize!");
 ...
 inline __attribute__((always_inline)) bool operator==(const LocationSize &Other) const {    return Value == Other.Value;
  }

 inline __attribute__((always_inline)) bool operator==(const TypeSize &Other) const {    return hasValue() && getValue() == Other;
  }

 inline __attribute__((always_inline)) bool operator!=(const LocationSize &Other) const { return !(*this == Other); }
 inline __attribute__((always_inline)) bool operator!=(const TypeSize &Other) const { return !(*this == Other); }
