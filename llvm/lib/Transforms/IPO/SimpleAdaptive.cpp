...
private:
    ...
    Function *createProfilingWrapper(FunctionMetadata &metadata, Module &M); // Profiling wrapper
    Function *createProductionWrapper(FunctionMetadata &metadata, Module &M); // Production wrapper
...
PreservedAnalyses SimpleAdaptivePass::run(Module &M, ModuleAnalysisManager &MAM) {
    SimpleAdaptivePassImpl Impl;
    return Impl.run(M, MAM);
}

PreservedAnalyses SimpleAdaptivePassImpl::run(Module &M, ModuleAnalysisManager &MAM) {
    bool modified = false;
...
Function *SimpleAdaptivePassImpl::createVersion(Function *Orig, int versionIndex, Module &M) {
...
Function *SimpleAdaptivePassImpl::createProfilingWrapper(FunctionMetadata &metadata, Module &M) {
    ...
    LoadInst *LoadComplete = Builder.CreateLoad(i32Ty, metadata.profilingComplete);
    ...
    Builder.SetInsertPoint(ProfilingBB);
    {
        ...
        for (int v = 0; v < 4; v++) {
            ...
            if (Orig->getReturnType()->isVoidTy()) {
                ...
                Value *VResult = Builder.CreateCall(metadata.versions[v]->getFunctionType(),
                                                   metadata.versions[v], Args);
        ...
        for (int v = 0; v < 4; v++) {
            UpdateBB[v] = BasicBlock::Create(Ctx, "update_v" + std::to_string(v), Wrapper);
            NextBB[v] = BasicBlock::Create(Ctx, "next_v" + std::to_string(v), Wrapper);
        }
        ...
        for (int v = 0; v < 4; v++) {
            ...
            Builder.SetInsertPoint(UpdateBB[v]);
            {
                ...
                Builder.CreateAtomicRMW(AtomicRMWInst::Add, metadata.cpuCycles[v],
                                       Elapsed, MaybeAlign(), AtomicOrdering::Monotonic);
                
                // Atomic add to cpuCyclesSquared
                Value *ElapsedSquared = Builder.CreateMul(Elapsed, Elapsed);
                Builder.CreateAtomicRMW(AtomicRMWInst::Add, metadata.cpuCyclesSquared[v],
                                       ElapsedSquared, MaybeAlign(), AtomicOrdering::Monotonic);
        ...
        Value *NextVersionMod = Builder.CreateURem(NextVersion, ConstantInt::get(i32Ty, 4));
        StoreInst *StoreNext = Builder.CreateStore(NextVersionMod, metadata.currentVersion);
    ...
    Builder.SetInsertPoint(FinalizeBB);
    {
        ...
        BasicBlock *SkipFinalizeBB = BasicBlock::Create(Ctx, "skip_finalize", Wrapper);
        ...
        Builder.SetInsertPoint(DoFinalizeBB);
        {
            ...
            for (int v = 0; v < 4; v++) {
                ...
                Value *CyclesF64 = Builder.CreateUIToFP(Cycles[v], Builder.getDoubleTy());
                Value *CyclesSqF64 = Builder.CreateUIToFP(CyclesSq[v], Builder.getDoubleTy());
                
                Value *HasRuns = Builder.CreateICmpSGT(Runs[v], ConstantInt::get(i32Ty, 0));
                Value *SafeRuns = Builder.CreateSelect(HasRuns, RunsF64,
            ...
            for (int v = 1; v < 4; v++) {
                ...
                Value *ShouldUpdate = Builder.CreateOr(
                    ...
                    Builder.CreateAnd(Builder.CreateAnd(CurrentReliable, BestReliable), IsFaster)
                );
                
                BestVersion = Builder.CreateSelect(ShouldUpdate,
                                                  ConstantInt::get(i32Ty, v), BestVersion);
            ...
            FunctionCallee WriteProfile = M.getOrInsertFunction(
                "__adaptive_write_profile", WriteProfileType);
        ...
        for (int v = 0; v < 4; v++) {
            BasicBlock *BB = (v == 0 ? BestV0BB : v == 1 ? BestV1BB :
                            v == 2 ? BestV2BB : BestV3BB);
    ...
    Builder.SetInsertPoint(OptimalBB);
    {
        ...
        for (int v = 0; v < 4; v++) {
            BasicBlock *BB = (v == 0 ? OptV0BB : v == 1 ? OptV1BB :
                            v == 2 ? OptV2BB : OptV3BB);
            Builder.SetInsertPoint(BB);
...
Function *SimpleAdaptivePassImpl::createProductionWrapper(FunctionMetadata &metadata, Module &M) {
    LLVMContext &Ctx = M.getContext();
    Function *Orig = metadata.original;
    ...
    LoadInst *LoadBest = Builder.CreateLoad(i32Ty, metadata.bestVersion);
    LoadBest->setAtomic(AtomicOrdering::Acquire);
    Value *BestIsZero = Builder.CreateICmpEQ(LoadBest, ConstantInt::get(i32Ty, 0));
    ...
    Builder.SetInsertPoint(InitBB);
    {
        ...
        FunctionCallee ReadProfile = M.getOrInsertFunction(
            "__adaptive_read_profile", ReadProfileType);
        
        Value *ProfiledBest = Builder.CreateCall(ReadProfile, {FuncName});
        
        // If not found (-1), default to V0
        Value *NotFound = Builder.CreateICmpEQ(ProfiledBest, ConstantInt::get(i32Ty, -1));
        Value *ActualBest = Builder.CreateSelect(NotFound, ConstantInt::get(i32Ty, 0), ProfiledBest);
        
        // Store it
        StoreInst *StoreBest = Builder.CreateStore(ActualBest, metadata.bestVersion);
        StoreBest->setAtomic(AtomicOrdering::Release);
...
Function *SimpleAdaptivePassImpl::createThinDispatcher(FunctionMetadata &metadata, Module &M) {
    ...
    if (Orig->getReturnType()->isVoidTy()) {
        Builder.CreateCall(metadata.wrapper->getFunctionType(),
...
void SimpleAdaptivePassImpl::processAdaptiveFunction(Function *F, Module &M) {
    ...
    if (isProductionMode()) {
        ...
        errs() << "[ADAPTIVE] Production mode: " << F->getName() << "\n";
...
void SimpleAdaptivePassImpl::createInitializationFunction(Module &M) {
    ...
    if (isProfilingMode()) {
        errs() << "[ADAPTIVE] Registering " << functionMap.size() << " function(s) for profiling\n";
        
        FunctionType *InitProfType = FunctionType::get(Type::getVoidTy(Ctx), {}, false);
        FunctionCallee InitProf = M.getOrInsertFunction(
            "__adaptive_init_profiling", InitProfType);
        Builder.CreateCall(InitProf);
        
        Type *i8PtrTy = PointerType::get(Type::getInt8Ty(Ctx), 0);
        Type *i32PtrTy = PointerType::get(Type::getInt32Ty(Ctx), 0);
        
        FunctionType *RegType = FunctionType::get(Type::getVoidTy(Ctx),
                                                 {i8PtrTy, i32PtrTy}, false);
        FunctionCallee RegFunc = M.getOrInsertFunction(
            "__adaptive_register_function", RegType);
        ...
        for (auto &pair : functionMap) {
            ...
            if (metadata.profilingComplete) {  // Only profiling mode has this
                ...
                Builder.CreateCall(RegFunc, {FuncName, metadata.profilingComplete});
            }
        }
    } else {
