...
namespace llvm
{
    ...
    // Structure to track function metadata - moved inside the class
    ...
    {
    private:
        // Non-static member variables instead of global static
        ...
        Function *createSharedDispatcher(Module &M);
        void processFunctionWithSharedDispatcher(Function *F, Function *SharedDispatcher,
                                                 uint32_t funcId, Module &M);
        void createInitializationFunction(Module &M);
        Function *createWrapperForSharedDispatcher(Function *Original,
                                                   Function *SharedDispatcher,
                                                   uint32_t funcId, Module &M);

    public:
        SimpleAdaptivePassImpl() : dispatchTable(nullptr), functionCounterTable(nullptr), functionCount(nullptr) {}
    ...
    {
        // Create a new instance for each module to avoid cross-module contamination
    ...
    PreservedAnalyses SimpleAdaptivePassImpl::run(Module &M, ModuleAnalysisManager &)
    {
        ...
        // Clear any previous state (defensive programming)
        functionMap.clear();
    ...
    void SimpleAdaptivePassImpl::createGlobalDispatchInfrastructure(Module &M)
    {
        ...
        // Check if globals already exist in this module (avoid duplicates)
        if (M.getGlobalVariable("__shared_dispatch_table"))
        {
            errs() << "  Dispatch infrastructure already exists, skipping creation\n";
            dispatchTable = M.getGlobalVariable("__shared_dispatch_table");
            functionCounterTable = M.getGlobalVariable("__function_counters");
    ...
    Function *SimpleAdaptivePassImpl::createSharedDispatcher(Module &M)
    {
        LLVMContext &Ctx = M.getContext();

        // Check if dispatcher already exists
        if (Function *existing = M.getFunction("__shared_dispatcher"))
        {
            errs() << "  Shared dispatcher already exists, reusing\n";
            return existing;
        }
        ...
        Value *MaxFuncId = Builder.CreateLoad(Type::getInt32Ty(Ctx), functionCount);
        Value *ValidId = Builder.CreateICmpULT(FuncId, MaxFuncId);

        BasicBlock *ValidBB = BasicBlock::Create(Ctx, "valid_func", Dispatcher);
        BasicBlock *ErrorBB = BasicBlock::Create(Ctx, "error", Dispatcher);

        Builder.CreateCondBr(ValidId, ValidBB, ErrorBB);

        // Error block
        Builder.SetInsertPoint(ErrorBB);
        Value *ErrorMsg = Builder.CreateGlobalStringPtr("[ERROR] Invalid function ID: %d\n");
        Builder.CreateCall(Printf, {ErrorMsg, FuncId});
        ...
        // Add null check for function pointer
        Value *IsNull = Builder.CreateICmpEQ(FuncPtr, ConstantPointerNull::get(PointerType::get(Type::getInt8Ty(Ctx), 0)));
        BasicBlock *CallBB = BasicBlock::Create(Ctx, "call_func", Dispatcher);
        BasicBlock *NullBB = BasicBlock::Create(Ctx, "null_func", Dispatcher);

        Builder.CreateCondBr(IsNull, NullBB, CallBB);

        // Null function pointer
        Builder.SetInsertPoint(NullBB);
        Value *NullMsg = Builder.CreateGlobalStringPtr("[ERROR] Null function pointer for ID %d\n");
        Builder.CreateCall(Printf, {NullMsg, FuncId});
        Builder.CreateRet(ConstantPointerNull::get(PointerType::get(Type::getInt8Ty(Ctx), 0)));
    ...
    void SimpleAdaptivePassImpl::processFunctionWithSharedDispatcher(Function *F, Function *SharedDispatcher,
                                                                     uint32_t funcId, Module &M)
    {
        LLVMContext &Ctx = M.getContext();
        ...
        errs() << metadata.versions[0]->getName() << " ," << metadata.versions[1]->getName() << "\n";
        errs() << metadata.wrapper->getName() << "\n";
    }
    ...
    Function *SimpleAdaptivePassImpl::createWrapperForSharedDispatcher(Function *Original,
                                                                       Function *SharedDispatcher,
                                                                       uint32_t funcId, Module &M)
    {
        LLVMContext &Ctx = M.getContext();
        ...
        Builder.CreateCall(SharedDispatcher, {ConstantInt::get(Type::getInt32Ty(Ctx), funcId),
                                              ArgsArray});
    ...
    Function *SimpleAdaptivePassImpl::createWrapper(Function *Original, Function *Dispatcher, Module &M)
    {
        Function *Wrapper = Function::Create(
            Original->getFunctionType(),
            Original->getLinkage(),
