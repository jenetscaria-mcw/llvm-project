...
namespace {
  ...
  inline __attribute__((always_inline)) class RISCVInstructionSelector : public InstructionSelector {public:
...
inline __attribute__((always_inline)) bool RISCVInstructionSelector::replacePtrWithInt(MachineOperand &Op,
                                                 MachineIRBuilder &MIB,
              MachineRegisterInfo &MRI) {  Register PtrReg = Op.getReg();
...
inline __attribute__((always_inline)) bool RISCVInstructionSelector::isRegInGprb(Register Reg,
            MachineRegisterInfo &MRI) const {  return RBI.getRegBank(Reg, MRI, TRI)->getID() == RISCV::GPRBRegBankID;
}

inline __attribute__((always_inline)) bool RISCVInstructionSelector::isRegInFprb(Register Reg,
            MachineRegisterInfo &MRI) const {  return RBI.getRegBank(Reg, MRI, TRI)->getID() == RISCV::FPRBRegBankID;
}
...
inline __attribute__((always_inline)) static unsigned getFCmpOpcode(CmpInst::Predicate Pred, unsigned Size) {  assert((Size == 16 || Size == 32 || Size == 64) && "Unsupported size");
  switch (Pred) {
  default:
...
static bool legalizeFCmpPredicate(Register &LHS, Register &RHS,
  ...
  inline __attribute__((always_inline)) auto isLegalFCmpPredicate = [](CmpInst::Predicate Pred) {    
